...
//#define __DI_EI__	//to define is interrupt has to be disabled and enabled to avoid data corruption
...
class AdcRingBuffer
{
	public:
		...
		int used()
			{
				int bufused;
					...
					register unsigned int idxMemRound = m_idxMemRound;

					register int idxGet = m_idxGet;
					register unsigned int idxGetRound = m_idxGetRound;
					asm("ei");
				#else
					register int idxMem = m_idxMem;
					register unsigned int idxMemRound = m_idxMemRound;
		...
		void clear() { m_count = m_idxMem = m_idxGet = m_idxMemRound = m_idxGetRound =0;  }
	...
	private:
		...
		unsigned int m_idxMemRound;
		unsigned int m_idxGetRound;
};
...
*/
extern PrsHwChanManager <word, _PRS_BUFFER_LENGTH_> *prsChan;
