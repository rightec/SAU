...
#include "SSPIE_intrface.h"
#include "SSPIE_opcode.h"
#include "SSPIE_debug.h"
#include "SSPIE_hardware.h"
#include "siosg2.h"
#include "global.h"

#define CS_EN() (PinCS_SPI_FPGA = 0)
#define CS_DI() (PinCS_SPI_FPGA = 1)
...
{
	char str[40];
	sprintf(str,"DbgCode:%d, DbgCode2:%d",debugCode, debugCode2);
...
int TRANS_transmitBytes(unsigned char *trBuffer, int trCount)
{
	...
	if (mod8 > 0)
	{
		...
		trBuffer[numByteTr-1] |= padder;
	}
	
	while( counter < numByteTr)
	{
		sio_putc_ch3(trBuffer[counter]);
		counter++;
	}
...
int TRANS_receiveBytes(unsigned char *rcBuffer, int rcCount)
{
	...
	if (mod8 > 0)
	{
		...
		rcBuffer[numByteRc-1] |= padder;
	}
	
	while( counter < numByteRc)
	{
		rcBuffer[counter] = sio_getc_ch3(filler);
		counter++;
	}
	return 1;
}
