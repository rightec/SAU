...
***************************************************************/
#include "global.h"
#include "SSPIE_SSPIEm.h"
#include "SSPIE_debug.h"
#include "SSPIE_SAUBRIDGE_ALGO.h"
#include "SSPIE_SAUBRIDGE_DATA.h"
...
int SSPIE_main( )
{
	...
	unsigned char *setAlgoPtr = (unsigned char*)g_pucAlgoArray;
	unsigned int setAlgoSize  = g_iAlgoSize; 
	unsigned char *setDataPtr = (unsigned char*)g_pucDataArray;
	unsigned int setDataSize  = g_iDataSize;
	
	print_out_string( "Lattice Semiconductor Corp.\n" );
	sprintf(Message,"\nSSPI Embedded(tm) V%s 2011",VME_VERSION_NUMBER);
	print_out_string( Message );

	siRetCode = SSPIEm_preset(setAlgoPtr, setAlgoSize, setDataPtr, setDataSize);
