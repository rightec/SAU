...
class HwChannel
{
	public:
		...
		*/
		HwChannel(bitResolution bits = _16_BITS_RESOLUTION_)
		{
		    m_fifoAdcData.clear();
...
class HwChanManager
{
	public:
		...
		*/
		inline bool popAdcData (T *adc, bool isr_di = true)
		{
			int i;
		...
		*/
		inline bool popAdcData (T *adc, int num, bool isr_di = true)
		{
			int i, k;
			bool readValues = false;
			int bufferData;
			...
			if (readValues)
			{
				for(i = 0; i < num; i++)
				{
					...
					baseIndex = i*m_numHwChan;
					for(k = 0; k < m_numHwChan; k++)
					{
						if (m_hwChan[k]->isEnabled())
						{
							if(isr_di)
							{
								asm("di");
								m_hwChan[k]->popAdcData(adc[baseIndex + k]);
								asm("ei");
							}else
							{
								m_hwChan[k]->popAdcData(adc[baseIndex + k]);
		...
		*/
		void clearFifo(bool isr_di = true)
		{
			int i;
			if(isr_di)
			{
